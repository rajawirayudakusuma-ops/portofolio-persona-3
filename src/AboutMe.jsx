...
const REVEAL_CONTENT = [
  {
    upper: ["Nama: Raja Wirayuda K", "Ilmu Komunikasi - UPI", "Bandung, Indonesia"],
    lower: "Mahasiswa dan Jurnalis Muda",
  },
  {
    upper: [
      "Wajah saya pernah muncul di Instagram resmi UPI.",
      "Ketua Komisi Disiplin yang ditakuti tapi disukai.",
      "IKOM UPI selalu jadi jurusan nomor 1 di UPI!",
    ],
    lower: "another fun fact :p",
  },
  {
    upper: [
      "Pernah angkat 130Kg di puncak performa fisik saya.",
      "Kontributor opini di IsolaPos, BandungBergerak, detikJabar.",
      "Komisi Disiplin 3x, lalu jadi Ketua Komisi Disiplin.",
    ],
    lower: "above is my wired fact",
  },
];
...
export default function AboutMe() {
  ...
  useEffect(() => {
    if (bgRef.current) requestBackground(ABOUT_VIDEO_SRC, bgRef.current);
  }, [requestBackground]);
  ...
  return (
    <div id="menu-screen" style={{ opacity: isReady ? 1 : 0, transition: 'opacity 300ms ease', pointerEvents: isReady ? 'all' : 'none' }}>
      ...
      <style>{`
        ...
        .sc-bar-outer:nth-child(1) { transition-delay: 0ms; z-index: 3; }
        .sc-bar-outer:nth-child(2) { transition-delay: 80ms; z-index: 2; }
        .sc-bar-outer:nth-child(3) { transition-delay: 160ms; z-index: 1; }
      ...
      <div className={`sc-footer${mounted ? " mounted" : ""}`}>
        ...
        >
          <span className="sc-footer-key">↵</span><span>REVEAL</span>
        </div>
        <div
          className="sc-footer-row"
          style={{cursor:'pointer', pointerEvents:'all'}}
        ...
        >
          <span className="sc-footer-key">ESC</span><span>BACK</span>
        </div>
      </div>
    </div>
  );
